...
- 目的は「CLIエージェント（Codex / Claude Code）から、ユーザーが普段使っている“ログイン済み”Chromeを確実に操作する」こと。
...
- ツール分裂は避けたい。Codex / Claude Code が“いつもの使い方”で安定して呼べる単一のCLI（＋背後のブリッジ）を提供する。
...
- 実装要件（What）
  ...
  - 切断検知と理由の伝播（拡張→daemon→CLI→エージェントログ）
  - 復旧コマンド（最低2つ）
    - reconnect（通信再確立）
...
Codex / Claude Code 運用メモ（UI確認を自動で回すための最低限）

前提

- UI確認・動作確認・画面遷移確認は原則 `human-browser` を使う（ログイン済み Chrome を直接操作する）。
- 操作は決定論的に `snapshot -> ref -> action` で行う。
- ref（`@eN`/`ref=eN`/`eN`）で操作する場合は必ず `--snapshot <snapshot_id>` を付ける（ズレたら取り直す）。
- 3ブラウザ運用（`personal` / `anymind` / `acua`）では profile ルーティングを使う。必要に応じて `--profile <name>` で強制できる。
...
human-browser profile resolve --hint "会社ブラウザ1で検証" --json

# 2) 対象ページへ
human-browser open https://example.com

# 3) 観測（操作候補だけに絞るのが基本）
human-browser snapshot --interactive --cursor --compact

# 4) ref で操作（snapshot の snapshot_id を必ず添える）
human-browser click @e12 --snapshot <snapshot_id>
human-browser fill  @e34 "hello@example.com" --snapshot <snapshot_id>

# 5) 必要なら再度 snapshot して次アクションへ
human-browser snapshot --interactive --cursor --compact
```
...
- `DISCONNECTED` のとき
  - まず `human-browser --json status` で extension 接続を確認
  - 必要なら `human-browser reconnect`、それでもダメなら `human-browser reset`
- `STALE_SNAPSHOT` のとき
  - `human-browser snapshot` を取り直して ref を更新してから再実行

補足

- macOS では daemon を LaunchAgent 常駐させる（`com.teramotodaiki.human-browser`）。
- `~/.local/bin/human-browser` ラッパーが daemon の `health` を見て落ちていれば自動復帰を試みる（`HUMAN_BROWSER_AUTOSTART=0` で無効化）。
- profile ルーティング設定は `~/.human-browser/profiles.json`。雛形は `docs/profiles.example.json`。
