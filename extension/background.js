...
const RECONNECT_ALARM_NAME = 'hb-reconnect';
const WATCHDOG_ALARM_NAME = 'hb-watchdog';
...
// --- Keepalive: SW-initiated message to reset idle timer (Chrome 116+) ---
function startKeepalive() {
  stopKeepalive();
  state.daemon.keepaliveTimer = setInterval(() => {
    if (state.socket && state.socket.readyState === WebSocket.OPEN) {
      sendSocket({ type: 'KEEPALIVE', ts: new Date().toISOString() });
    }
  }, KEEPALIVE_INTERVAL_MS);
}

function stopKeepalive() {
  if (state.daemon.keepaliveTimer) {
    clearInterval(state.daemon.keepaliveTimer);
    state.daemon.keepaliveTimer = null;
  }
}

// --- Alarms: insurance reconnect when SW wakes from suspension ---
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name === RECONNECT_ALARM_NAME) {
    // One-shot reconnect alarm fired; attempt connect
    connectSocket();
  }
  if (alarm.name === WATCHDOG_ALARM_NAME) {
    // Periodic watchdog: if disconnected, try to reconnect
    if (!state.socket || state.socket.readyState !== WebSocket.OPEN) {
      connectSocket();
    }
  }
...
function connectSocket(options = { force: false }) {
  ...
  socket.onopen = () => {
    ...
    // Start SW-initiated keepalive to reset Chrome's idle timer
    startKeepalive();
  };
...
function scheduleReconnect() {
  ...
  const delayMs = Math.min(RETRY_MAX_MS, RETRY_BASE_MS * (2 ** Math.max(0, state.daemon.retryCount - 1)));

  // Use setTimeout for short delays (SW is still alive right after disconnect)
  // AND schedule an alarm as insurance in case SW suspends before setTimeout fires
  state.daemon.reconnectTimer = setTimeout(() => {
    connectSocket();
  }, delayMs);

  // Alarm as backup: minimum 0.5 min (30s) in production, but ensures reconnect even after SW suspension
  const alarmDelayMin = Math.max(0.5, delayMs / 60000);
  chrome.alarms.create(RECONNECT_ALARM_NAME, { delayInMinutes: alarmDelayMin });
}
