...
```bash
git clone https://github.com/omochanomachacha/browser4Claude-Codex.git ~/.human-browser
cd ~/.human-browser
npm install
npm link
...
ポート重複を避けて profile ごとに `init` します。

```bash
human-browser --config ~/.human-browser/config.personal.json init --port 18765 --show-token
human-browser --config ~/.human-browser/config.anymind.json init --port 18766 --show-token
human-browser --config ~/.human-browser/config.acua.json init --port 18767 --show-token
```
...
```bash
launchctl bootstrap "gui/$(id -u)" ~/Library/LaunchAgents/com.teramotodaiki.human-browser.personal.plist
launchctl bootstrap "gui/$(id -u)" ~/Library/LaunchAgents/com.teramotodaiki.human-browser.anymind.plist
launchctl bootstrap "gui/$(id -u)" ~/Library/LaunchAgents/com.teramotodaiki.human-browser.acua.plist

launchctl kickstart -k "gui/$(id -u)/com.teramotodaiki.human-browser.personal"
launchctl kickstart -k "gui/$(id -u)/com.teramotodaiki.human-browser.anymind"
launchctl kickstart -k "gui/$(id -u)/com.teramotodaiki.human-browser.acua"
```
...
- URLがないコマンドでヒントを与える場合は `HUMAN_BROWSER_CONTEXT_HINT` を使えます（例: `HUMAN_BROWSER_CONTEXT_HINT='会社ブラウザ1' human-browser status`）。
...
- profile 状態確認/操作:
  ...
  - `human-browser profile use anymind --json`
  - `human-browser profile resolve --url https://example.com --hint "会社ブラウザ1" --json`
...
既存環境で `anymind` が `~/.human-browser/config.json` を使っている場合は、そのまま `profiles.json` 側で割り当てても問題ありません。
...
HUMAN_BROWSER_CONTEXT_HINT="acuaで確認" human-browser --expect-selector "#app" open https://acua.ai
```
...
human-browser profile list --json
human-browser profile current --json
human-browser profile resolve --hint "personal" --json
human-browser --profile personal --json status
human-browser --profile personal open https://example.com
human-browser --profile personal snapshot --interactive --cursor --compact
human-browser --profile personal diagnose --limit 20
```
